...
export default function Navbar({ profileName, profileId, profilePhoto, onSignOut }: NavbarProps) {
  ...
  const navItems = [
    ...
    { name: 'Chợ sinh viên', path: '/market', icon: (
      <svg className="w-7 h-7" fill="currentColor" viewBox="0 0 24 24"><path d="M20 6h-4V4c0-1.11-.89-2-2-2h-4c-1.11 0-2 .89-2 2v2H4c-1.11 0-1.99.89-1.99 2L2 19c0 1.11.89 2 2 2h16c1.11 0 2-.89 2-2V8c0-1.11-.89-2-2-2zm-6 0h-4V4h4v2z"/></svg>
    )},
    ...
    <nav className="bg-white border-b border-slate-200 sticky top-0 z-50 h-16 flex items-center justify-between px-4 shadow-sm">
      ...
      <div className="flex items-center gap-4 w-1/3">
        <Link href="/dashboard" className="shrink-0 flex items-center">
          <Image src="/logo_ftu_don_gian.png" alt="FTU Connect" width={200} height={200} className="h-10 sm:h-14 w-auto object-contain cursor-pointer transition-transform hover:scale-105" />
        </Link>
        <div className="hidden lg:flex items-center bg-[#f0f2f5] rounded-full px-3 py-2 w-64 h-10 shrink-0 ml-2">
