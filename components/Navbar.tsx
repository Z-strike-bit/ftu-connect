...
export default function Navbar({ profileName, profileId, onSignOut }: NavbarProps) {
  ...
  return (
    <nav className="bg-white border-b border-slate-200 sticky top-0 z-50 h-14 flex items-center justify-between px-4 shadow-sm">
      {/* Left: Logo + Search */}
      <div className="flex items-center gap-2 w-1/4">
        <Link href="/dashboard" className="shrink-0 flex items-center pl-2 ml-2">
          <Image src="/logo.png" alt="FTU Connect" width={240} height={80} className="h-12 sm:h-14 w-auto object-contain scale-[2] sm:scale-[2.5] origin-left" />
        </Link>
        <div className="hidden lg:flex items-center bg-[#f0f2f5] rounded-full px-3 py-2 ml-16 xl:ml-24 w-64 h-10">
      ...
      <div className="flex items-center justify-end gap-2 w-1/4">
        ...
        <Link href={profileId ? `/profile/${profileId}` : "#"} className="w-10 h-10 rounded-full bg-[#e4e6eb] flex items-center justify-center hover:bg-[#d8dadf] transition-colors cursor-pointer">
