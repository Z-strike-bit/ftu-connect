...
export default function Navbar({ profileName, profileId, profilePhoto, onSignOut }: NavbarProps) {
  ...
  return (
    <nav className="bg-white border-b border-[#ebebeb] sticky top-0 z-50 h-20 flex items-center justify-between px-6 sm:px-10 shadow-[0_1px_2px_rgba(0,0,0,0.04)]">
      {/* Left: Logo */}
      <div className="flex items-center w-1/4">
        <Link href="/dashboard" className="shrink-0 flex items-center">
          <Image src="/logo_ftu_don_gian.png" alt="FTU Connect" width={200} height={200} className="h-12 sm:h-14 w-auto object-contain cursor-pointer transition-transform hover:scale-105" />
        </Link>
      </div>
          ...
          return (
            <Link 
              ...
              className={`relative flex items-center justify-center h-full px-2 transition-colors
                ${isActive ? 'text-[#ff385c]' : 'text-[#6a6a6a] hover:text-[#222222] hover:bg-[#f7f7f7] rounded-lg my-3'}
              `}
            >
              {item.icon}
              {isActive && (
                <div className="absolute bottom-0 left-0 w-full h-[3px] bg-[#ff385c] rounded-t-md" />
              )}
            </Link>
          );
        })}
      ...
      <div className="flex items-center justify-end gap-4 w-[35%]">
        <div className="hidden lg:flex items-center bg-white border border-[#dddddd] shadow-[0_1px_2px_rgba(0,0,0,0.08),0_4px_12px_rgba(0,0,0,0.05)] rounded-full px-2 py-1.5 hover:shadow-[0_2px_4px_rgba(0,0,0,0.18)] transition-shadow cursor-pointer h-12 w-full max-w-[280px]">
          <input type="text" placeholder="Tìm kiếm bất kỳ..." className="bg-transparent border-none outline-none text-[14px] font-medium w-full placeholder-[#6a6a6a] text-[#222222] px-3" />
          <div className="bg-[#ff385c] rounded-full w-8 h-8 flex items-center justify-center shrink-0">
            <svg className="w-4 h-4 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth="3"><path strokeLinecap="round" strokeLinejoin="round" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z"/></svg>
          </div>
        </div>

        <div className="flex items-center gap-2 border border-[#dddddd] rounded-full p-1.5 hover:shadow-md transition-shadow bg-white ml-2">
          <button onClick={onSignOut} title="Menu" className="p-1 text-[#222222] hover:bg-[#f7f7f7] rounded-full">
            <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M4 6h16M4 12h16M4 18h16" /></svg>
          </button>
          <Link href={profileId ? `/profile/${profileId}` : "#"} className="flex items-center gap-2">
            <div className="w-8 h-8 rounded-full bg-[#6a6a6a] flex items-center justify-center overflow-hidden">
              {profilePhoto ? (
